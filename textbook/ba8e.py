...
def upgma(d,n):
    ...
    while len(clusters) > 1:
        ...
        clusters.append(c_new); ans.append(c_new)
    return ans

with open('./testcase/rosalind_ba8e.txt','r') as f:
    n = int(f.readline())
    d = [list(map(float,s.split())) for s in f.readlines()]
    print('\n'.join(' '.join(str(x+1) for x in c) for c in upgma(d,n)))
